...
function nodeDefinitions(idFetcher, typeResolver) {
  ...
  const nodeField = {
    ...
    resolve: (_obj, {
      id
    }, context, info) => idFetcher(id, context, info)
  };
  const nodesField = {
    ...
    resolve: (_obj, {
      ids
    }, context, info) => ids.map(id => idFetcher(id, context, info))
  };
  return {
    nodeInterface,
    nodeField,
...
function globalIdField(typeName, idFetcher) {
  return {
    description: 'The ID of an object',
    type: new _graphql.GraphQLNonNull(_graphql.GraphQLID),
    resolve: (obj, _args, context, info) => toGlobalId(typeName !== null && typeName !== void 0 ? typeName : info.parentType.name, idFetcher ? idFetcher(obj, context, info) : obj.id)
  };
}
